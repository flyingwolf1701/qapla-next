...
export function MovementSessionControls({ movementCategory, initialUserLevel, onMovementComplete }: MovementSessionControlsProps) {
  ...
  useEffect(() => {
    ...
    if (movementCategory.id === 'push' || movementCategory.id === 'legs' || movementCategory.id === 'dips') {
    ...
    } else if (movementCategory.id === 'pull' || movementCategory.id === 'core') {
  ...
  }, [movementCategory, initialUserLevel]); // Only re-run full reset on category change
  ...
  useEffect(() => {
    ...
    sessionFixedExerciseTypeIsRepBased, movementCategory, updateUserLevel, upArrowIconIsLock, LEVEL_UP_THRESHOLD_REPS
  ]);
  ...
  const handleTimeBasedLevelChange = useCallback((direction: 'up' | 'down') => {
    ...
    // Log current time if any, before changing level
    if (currentElapsedTime > 0) {
    ...
    setCurrentElapsedTime(0); // Reset for new level/wave
    ...
    if (relevantProgressions.length === 0) {
      ...
      setTimerKey(Date.now()); // Reset timer even if level doesn't change
      return;
    }
    ...
    currentElapsedTime, waveNumber, updateUserLevel
  ]);
  ...
  const downArrowDisabled = useMemo(() => {
    if (!currentMovementDetails || !sessionFixedExerciseTypeIsRepBased) return true;
    const relevantProgressions = movementCategory.progressions
      .filter(p => p.isRepBased && p.level > 0)
      .sort((a, b) => a.level - b.level);
        ...
        {sessionFixedExerciseTypeIsRepBased && (
          <>
            <div className="flex items-center justify-center gap-2 mt-2">
              ...
              <Button variant="outline" size="icon" onClick={() => handleArrowLevelChange('up')} disabled={upArrowDisabled} aria-label="Next Level & Log Wave">
        ...
        {!sessionFixedExerciseTypeIsRepBased && currentMovementDetails && (
          <div className="space-y-4">
            ...
            <Timer
              ...
              autoStart={false} // User explicitly starts
              className="py-0"
            />
            {levelUpHintText && <p className="text-xs text-muted-foreground text-center mt-1">{levelUpHintText}</p>}
